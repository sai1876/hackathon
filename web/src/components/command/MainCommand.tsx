...
function DatabaseMap({polygon,drawing,onPoint,onFinish,onUndo,focus,electric=false}:{focus?:number[][];electric?:boolean;polygon:number[][];drawing:boolean;onPoint:(point:number[])=>void;onFinish:()=>void;onUndo:()=>void}) {
  ...
  useEffect(()=>{if(!signalMap?.getSource("scenario-draft"))return;const features:FeatureCollection['features']=polygon.map((coordinates,index)=>({type:'Feature',properties:{index:index+1},geometry:{type:'Point',coordinates}}));if(polygon.length>1)features.unshift({type:'Feature',properties:{},geometry:{type:'LineString',coordinates:polygon}});(signalMap.getSource("scenario-draft") as maplibregl.GeoJSONSource).setData({type:'FeatureCollection',features});},[polygon,signalMap]);
  ...
  useEffect(() => {
    ...
    map.on("style.load", () => { map.addSource("scenario",{type:"geojson",data:{type:"FeatureCollection",features:[]}});map.addLayer({id:"scenario-fill",type:"fill",source:"scenario",paint:{"fill-color":["case",["get","blocked"],"#ed6170",["get","slow"],"#e0a844","#479fdd"],"fill-opacity":0.26}});map.addLayer({id:"scenario-outline",type:"line",source:"scenario",paint:{"line-color":["case",["get","blocked"],"#ff7d88",["get","slow"],"#f0c06f","#7dccff"],"line-width":2}});map.addSource("scenario-draft",{type:"geojson",data:{type:"FeatureCollection",features:[]}});map.addLayer({id:"scenario-draft-line",type:"line",source:"scenario-draft",filter:['==',['geometry-type'],'LineString'],paint:{'line-color':'#1d4ed8','line-width':3,'line-dasharray':[2,1]}});map.addLayer({id:"scenario-draft-points",type:"circle",source:"scenario-draft",filter:['==',['geometry-type'],'Point'],paint:{'circle-radius':7,'circle-color':'#ffffff','circle-stroke-color':'#1d4ed8','circle-stroke-width':3}});setSignalMap(map); map.addSource("roads", {type:"geojson",data:{type:"FeatureCollection",features:[]}}); map.addLayer({id:"roads",type:"line",source:"roads",minzoom:15,layout:{visibility:"none"},paint:{"line-color":["match",["get","traffic_status"],"BLOCKED","#ee4558","CONGESTED","#f97316","SLOW","#efbd38","#91a4ad"],"line-width":["interpolate",["linear"],["zoom"],15,2,18,5],"line-opacity":0.8}}); });
    map.on("zoom", () => {
  ...
  return <div className="atlas-map-content">{drawing&&<div className="scenario-map-drawing" role="status"><span><strong>Draw affected area</strong> · {polygon.length} point{polygon.length===1?'':'s'}. Click streets to add corners.</span><div><button disabled={!polygon.length} onClick={onUndo}>Undo point</button><button disabled={polygon.length<3} onClick={onFinish}>Finish area</button></div></div>}<div className="main-world-map" ref={container} aria-label="Database road map"/><p className="main-map-status">{status}</p><TrafficSignals map={signalMap}/>{!electric&&<MetroLayer map={signalMap}/>}<ElectricLayer map={signalMap}/></div>;
}
...
export default function MainCommand({ department = "main" }: { department?: "main" | "electric" | "field" }) {
 ...
 return <main className="main-command command-reference atlas-command">
  ...
  {(error||sharedError||shared&&!shared.ready)&&<p className="main-warning" role="alert">{sharedError||error||`Simulation database is degraded. ${shared?.failure??'Changes are locked until persistence recovers.'}`}</p>}
  <div className="atlas-stage">
   ...
   <section className="atlas-map-panel city-map-column" aria-label="Operational map workspace">
    ...
    <DatabaseMap electric={department!=='main'} focus={focus} polygon={polygon} drawing={drawing} onPoint={p=>setPolygon(old=>[...old,p])} onUndo={()=>setPolygon(old=>old.slice(0,-1))} onFinish={()=>setDrawing(false)}/>
